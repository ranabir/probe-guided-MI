...
def html():
    ...
    *{box-sizing:border-box;}
    ...
    /* interactive research-flow accordion */
    .flowlist{counter-reset:f;margin:14px 0;}
    .flow-item{position:relative;margin:10px 0;background:var(--card);border:1px solid var(--border);
               border-radius:10px;counter-increment:f;overflow:hidden;transition:box-shadow .15s;}
    .flow-item[open]{box-shadow:0 2px 10px rgba(20,20,19,0.06);border-color:#DAD3C4;}
    .flow-item summary{list-style:none;cursor:pointer;padding:14px 44px 14px 54px;font-size:16px;
                       position:relative;user-select:none;}
    .flow-item summary::-webkit-details-marker{display:none;}
    .flow-item summary::before{content:counter(f);position:absolute;left:16px;top:12px;width:26px;height:26px;
    ...
    .flow-item summary::after{content:'+';position:absolute;right:18px;top:12px;font-size:20px;color:var(--muted);
                     line-height:1;transition:transform .15s;}
    .flow-item[open] summary::after{content:'−';color:var(--coral);}
    .flow-item summary:hover{background:#F1ECE1;}
    .flow-item .lead{font-weight:650;}
    .flow-item .detail{padding:2px 22px 16px 54px;color:var(--muted);font-size:15px;line-height:1.62;}
    .flow-item .detail code{background:#EFEADF;padding:1px 5px;border-radius:4px;font-size:12.5px;color:var(--ink);}
    .flow-item .detail strong{color:var(--ink);}
    /* setup detail cards */
    .detailcards{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin:22px 0;}
    @media(max-width:640px){.detailcards{grid-template-columns:1fr;}}
    .dcard{background:var(--panel);border:1px solid var(--border);border-radius:12px;padding:18px 20px;}
    .dcard h4{margin:0 0 8px;color:var(--cloth);font-size:16px;}
    .dcard p{font-size:14.5px;margin:6px 0;} .dcard ul{font-size:14.5px;margin:6px 0;padding-left:18px;}
    .dcard li{margin:4px 0;} .dcard code{background:#EFEADF;padding:1px 5px;border-radius:4px;font-size:12.5px;}
  ...
  <div class="index">
    <h4>Index</h4>
    <ol>
      <li><a href="#flow">The research flow</a> — the 7-step story, click any step to expand <span class="tag">below</span></li>
      <li><a href="#setup">Setup &amp; technical details</a> — data, models, hooks, probe &amp; metrics <span class="tag">below</span></li>
  ...
  <p class="muted" style="font-size:15px;margin-top:0">Click any step to expand the details.</p>
  <div class="flowlist">
    <details class="flow-item">
      <summary><span class="lead">Measure the model's own preference</span> → is the signal real?</summary>
      <div class="detail">Rather than a hand label, the target is the model's <em>own</em> preference:
      <code>behavior_margin = mean-token logP(sycophantic | prompt) − logP(honest | prompt)</code>. We read
      the activation at the <strong>final prompt token</strong> — before any answer is generated — so the
      probe must detect an <em>upcoming</em> disposition, not the style of a visible answer. This choice
      also fixed an earlier invalidity: with the response visible, two rows of the same prompt had
      identical activations but opposite labels. Regression on the margin makes the target well-posed.</div>
    </details>
    <details class="flow-item">
      <summary><span class="lead">It's decodable — and it replicates</span> → could it be a trivial confound?</summary>
      <div class="detail">A per-layer linear probe predicts the margin, and its accuracy <strong>rises with
      depth</strong>: GPT-2 peaks at r≈0.52 (layer 8), Pythia at r≈0.61 (layer 16). The <em>same rising
      shape in two unrelated architectures</em> is the key — a shared, non-accidental mechanism. But
      "decodable" alone could still be a surface artifact, which motivated controls.</div>
    </details>
    <details class="flow-item">
      <summary><span class="lead">Controls say it's real; instruct models are worse</span> → where do we intervene?</summary>
      <div class="detail">Re-running the identical probe on a <strong>random-label</strong> target (shuffled
      = noise) gives a floor of ≈0.25; the real probe (≈0.52) clears it — genuine signal. Caveat: a pure
      surface feature (&ldquo;do you agree?&rdquo;) is also highly decodable, so a confound contribution
      isn't fully excluded. Separately, <strong>Qwen-Instruct is sycophantic 66% of the time vs ~50% for
      base models</strong> — post-training amplifies it.</div>
    </details>
    <details class="flow-item">
      <summary><span class="lead">Decoding ≠ causation</span> → intervene at causal layers, but with side effects?</summary>
      <div class="detail">Sweeping every layer independently, the <em>most readable</em> layer (≈L8) is
      <strong>not</strong> the <em>most causal</em> one (≈L1): reading and steering live in different
      places. So "intervene where the probe reads best" is the wrong instinct — attribute where it's
      causal. This immediately raised the reviewer's question: once we invoke a causal effect, does it
      damage the model?</div>
    </details>
    <details class="flow-item">
      <summary><span class="lead">Probe-direction steering works, but makes the model weird</span> → maybe it's not one direction</summary>
      <div class="detail">Additive steering <code>h ← h + α·d̂</code> flips up to ~57% of answers on Qwen —
      but it inflates the residual norm and the model becomes repetitive/incoherent (side-effect
      0.42–0.71). <strong>Activation capping</strong> is the opposite: clean (~0.04) but almost no flips.
      This fork — effective-but-disruptive vs clean-but-useless — pointed at the deeper hypothesis: perhaps
      sycophancy isn't a single direction.</div>
    </details>
    <details class="flow-item">
      <summary><span class="lead">Sycophancy is a subspace</span> → what are the extra directions?</summary>
      <div class="detail">Remove a low-rank subspace instead of one direction: <code>h ← h − (h Vᵀ)V</code>,
      with V the top-k singular vectors of many bootstrap difference-of-means directions. On Qwen a single
      direction flips only 17.5%, but a <strong>rank-2 subspace flips 40% at side-effect 0.03</strong> —
      ~2× the control, ~15× cleaner than blunt steering, and touching only 2 of ~900 dimensions. The
      near-&ldquo;golden&rdquo; case: reduce sycophancy without breaking the model.</div>
    </details>
    <details class="flow-item">
      <summary><span class="lead">One direction per topic</span> — the mechanistic explanation</summary>
      <div class="detail">Measuring how much of each sub-type's own direction lives in the subspace: the
      <strong>dominant direction is <em>political</em></strong> (captured at rank 1), the <strong>second
      is <em>NLP</em></strong> (0→0.55 at rank 2, which is why rank-2 doubles the flip), and
      <strong>philosophy stays diffuse</strong> (≤0.22, honestly capping the flip near 40%). Sycophancy is
      a <em>union of topic-specific directions</em> — one direction removes one topic; a subspace removes
      several at once.</div>
    </details>
  </div>
  ...
  {sec("", "Setup &amp; technical details", "setup")}
  <div class="detailcards">
    <div class="dcard">
      <h4>📊 Data</h4>
      <p>Anthropic <code>model-written-evals</code> sycophancy set, three sub-types pooled evenly:
      <code>philpapers2020</code> (philosophy), <code>nlp_survey</code> (NLP researchers),
      <code>political_typology_quiz</code> (politics), ~10k pairs each. We sample 300–500 prompts; each is
      a persona + stated view + a two-choice question with a sycophantic vs. honest answer.</p>
      <p><strong>Side-effect set:</strong> 30 authored general-knowledge prompts (&ldquo;Why is the sky
      blue?&rdquo;, &ldquo;What is 2+2?&rdquo;) in <code>data/side_effect_eval/basic_prompts.jsonl</code>.</p>
    </div>
    <div class="dcard">
      <h4>🧠 Models</h4>
      <ul>
        <li><strong>GPT-2 small</strong> — 12 layers, TransformerLens (base)</li>
        <li><strong>Pythia-410M</strong> — 24 layers, TransformerLens (base)</li>
        <li><strong>Qwen2.5-0.5B-Instruct</strong> — 24 layers, HuggingFace (instruct)</li>
        <li>Gemma-2 &amp; OLMo staged checkpoints — registry placeholders</li>
      </ul>
      <p>Per-model backend/dtype/stage live in <code>model_registry.yaml</code>.</p>
    </div>
    <div class="dcard">
      <h4>🪝 Hooks — read &amp; write</h4>
      <p>We attach hooks to each layer to both <em>read</em> activations (caching) and <em>edit</em> them
      (interventions):</p>
      <ul>
        <li><strong>TransformerLens</strong> (GPT-2, Pythia): named hook points, e.g.
        <code>blocks.L.hook_resid_post</code>.</li>
        <li><strong>HuggingFace</strong> (Qwen, Gemma): PyTorch <code>register_forward_hook</code> on the
        decoder-layer modules — a read/write wiretap on the residual stream. This is what makes instruct
        models steerable.</li>
      </ul>
      <p>Edits are applied at the final prompt-token position, then we re-measure behavior.</p>
    </div>
    <div class="dcard">
      <h4>🔬 Probe &amp; metrics</h4>
      <p><strong>Probe:</strong> ridge regression per layer on the prompt-final residual →
      <code>behavior_margin</code>; the attribution layer is chosen by a policy (e.g. best late layer).</p>
      <p><strong>Metrics:</strong> targeted syc→honest <em>flip rate</em> (did the A/B answer change?),
      <em>side-effect score</em> ∈ [0,1] (weirdness + length deviation + repetition + basic-QA drop), and
      <em>decodability</em> (probe test Pearson) — all with 95% bootstrap CIs.</p>
    </div>
  </div>
  """
