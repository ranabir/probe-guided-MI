...
LOG="logs/pythia_rebuild_maxlen256.log"
mkdir -p logs
...
python3 scripts/01c_build_balanced_preference_set.py --model_name "$M" --input data/processed/EleutherAI_pythia-410m_prompt_preferences.csv >> "$LOG" 2>&1
...
python3 -c "
import pandas as pd
m = pd.read_csv('results/tables/EleutherAI_pythia-410m_prompt_preferences_prompt_final_layer_probe_metrics.csv')
print('best test Pearson:', round(m['test_pearson'].max(),3), 'at layer', int(m.loc[m['test_pearson'].idxmax(),'layer']))
p = pd.read_csv('data/processed/EleutherAI_pythia-410m_prompt_preferences.csv')
